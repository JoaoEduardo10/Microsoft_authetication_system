...
import { GetTokenController } from "../../src/app/controllers/user/get-token";
...
describe("get-token", () => {
  it("should return the user's token", async () => {
    const repository = new MockGetUserRepository();
    const controller = new GetTokenController(repository);
